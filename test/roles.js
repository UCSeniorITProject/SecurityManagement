...
describe("Role API", async function () {
  ...
  describe("POST /api/role", async () => {
    it("succesfully creates a new role", async () => {
      const newRole = await roleHelpers.createRole(
        fastify,
    ...
    it("rejects an invalid request body", async () => {
      ...
      assert.strictEqual(
        roleRequest.statusCode,
        400,
        "Invalid request body allowed through API"
      );
    });
  });
  ...
  describe("PATCH /api/role/:id", async () => {
    it("succesfully updates already created role", async () => {
      ...
      const updateRoleRequest = await roleHelpers.updateRole(
        fastify,
        newFakeRole,
        JSON.parse(newRole.body).role.id,
        token
      );
      assert.strictEqual(
        updateRoleRequest.statusCode,
        200,
        "Created role was not updated"
      );
      assert.strictEqual(
        JSON.parse(updateRoleRequest.body).role.roleName,
        newFakeRole.roleName,
        "Updated role did not update name properly"
      );
    });
  ...
  describe("GET /api/role/list", async () => {
    it("successfully retrieves role after it is created", async () => {
      ...
      const createdRole = JSON.parse(roleList.body).roles.filter(
        (x) => x.roleName === role.roleName
      );
      assert.ok(
        createdRole.length > 0,
        "Role was not succesfully fetched or created"
      );
    });
    ...
    it("does not show role in list when deleted", async () => {
      ...
      assert.strictEqual(
        ...
        0,
        "Role was not succesfully deleted"
      );
    });
  });
  ...
  describe("DELETE /api/role/:id", async () => {
    ...
    it("returns 404 when the role is not found", async () => {
      const roleDeleteRequest = await roleHelpers.deleteRole(
        fastify,
        123112312,
        token
      );
      assert.strictEqual(
        roleDeleteRequest.statusCode,
        404,
        "404 was not returned"
      );
    });
  });
});
