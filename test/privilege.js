...
describe('Privilege API', async () => {
  ...
  describe('PATCH /api/privilege/:id', async () => {
    it('successfully updates a privilege', async () => {
      ...
      const createdPrivilege = await privilegeHelpers.create(fastify, mockPrivilege, token);
      ...
      const updatedPrivilegeRequest = await privilegeHelpers.update(fastify, parsedCreatedPrivilege.id, newPrivilege, token);
    ...
    it('throws a 404 when an invalid privilege is attempted to be updated', async () => {
      const updatedPrivilegeRequest = await privilegeHelpers.update(fastify, 1231123, {}, token);
  ...
  describe('DELETE /api/privilege/:id', async () => {
    ...
    it('returns 404 on invalid privilege', async () => {
      const deletedPrivilegeRequest = await privilegeHelpers.delete(fastify, 1231231213, token);
      assert.strictEqual(deletedPrivilegeRequest.statusCode, 404, 'Invalid privilege was allowed to be deleted');
    });
  });
});